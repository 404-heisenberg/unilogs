...
times so the numbers can be reproduced. Everything here is in the backend; the
frontend performance work lives in the D-issues.
...
search under 100 ms on this dataset — are both met. The entries numbers move a
few milliseconds between runs but stay an order of magnitude under budget.
...
<AiDeclaration
  tools="opencode[deepseek-v4.1-flash]"
  purpose="drafting this page from the backend benchmark results, the index migration and the EXPLAIN ANALYZE plans"
/>
