...
| ----------- | ------------- | ------------- | ---------------------------------------------------- |
| `frontend/` | `npm install` | `npm run dev` | `npm run lint`, `npm run typecheck`, `npm run build` |
...
**`frontend/tsconfig.json` is solution-style** (`"files": []`, only
`references`). Plain `npx tsc --noEmit` checks nothing against it and exits 0
even with real type errors elsewhere in `src/`. Always use `npm run typecheck`
(`tsc -b`), which follows the references into `tsconfig.app.json` — that's
also what `npm run build` and CI actually run.
